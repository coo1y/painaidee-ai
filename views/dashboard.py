"""PaiNaiDee-AI — 📊 Dashboard page (Streamlit multipage app).
...
Called via ``dashboard.render()`` from ``streamlit_app.py``.
"""
...
_THAI_FONT = _ROOT / "assets" / "fonts" / "Sarabun-Regular.ttf"
...
def render() -> None:
    """Render the monitoring dashboard (called by streamlit_app.py)."""
    st.title("📊 PaiNaiDee-AI — Monitoring Dashboard")

    df = fetch_df()

    if df.empty:
        st.info("No interactions logged yet. Chat on the **🧭 Chat** page first.")
        st.stop()
    ...
    with f2:
        ...
        selected_sentiments = st.multiselect(
            "Sentiment", options=_options, default=_options
        )
    ...
    with r1c2:
        ...
        daily = (
            df.assign(day=df["ts"].dt.date).groupby("day").size().reset_index(name="queries")
        )
    ...
    with r2c2:
        ...
        for c in comments:
            words += [
                w
                for w in re.findall(r"[a-zA-Z]{3,}", str(c).lower())
                if w not in _STOP
            ]
        if words:
    ...
    if df.empty:
        ...
        view["sentiment"] = (
            view["rating"].map({1: "👍 Positive", 0: "👎 Negative"}).fillna("—")
        )
